...
if ($(([datetime]::Parse($creds.issued)).AddSeconds($creds.expires_in)) -lt (Get-Date).AddSeconds(-10)) {
    $RefreshTokenParams = @{
...
$Attachments.ForEach( {
        ...
        try {
            $content = Get-Content -Path $_ -Raw -Encoding byte
        }
        catch {
            Write-Error "Attachment '$_' is not readable"
        }
